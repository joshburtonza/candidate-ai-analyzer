...
export const ResumeListItem = ({ upload, onDelete }: ResumeListItemProps) => {
  ...
  // Convert resume to display format
  const data = {
    candidate_name: upload.name,
    email_address: upload.email,
    contact_number: upload.phone,
    skill_set: upload.skills,
    score: upload.fit_score?.toString() || '0',
    countries: upload.nationality || upload.location || ''
  };
  ...
  const handleDelete = async (e: React.MouseEvent) => {
    ...
    console.log('ResumeListItem: Delete button clicked for candidate:', upload.id, data.candidate_name);
    
    const success = await deleteCandidate(upload.id, data.candidate_name || 'Unknown');
    
    console.log('ResumeListItem: Delete operation result:', success);
    
    if (success && onDelete) {
      console.log('ResumeListItem: Calling onDelete callback');
      onDelete(upload.id);
    } else if (!success) {
      console.error('ResumeListItem: Delete operation failed');
    } else if (!onDelete) {
      console.warn('ResumeListItem: No onDelete callback provided');
    }
  };
